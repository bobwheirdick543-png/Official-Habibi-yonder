...
export const LEVELS = [
    { threshold: 500, name: 'Lurker' },
    { threshold: 2000, name: 'Yapper' },
    { threshold: 5000, name: 'Certified Yapper' },
    { threshold: 9500, name: 'Group Chat Menace' },
    { threshold: 20000, name: "Habibi's Favorite" },
    { threshold: 50000, name: 'Living Legend' },
    { threshold: 100000, name: 'God Level' }
]
...
export async function incrementTextCount(memberId, groupId) {
    ...
    if (newRewardTier > previousRewardTier) {
        ...
        rewards.push({ type: 'text_reward', amount: rewardAmount })
    }

    let newLevel = 0
    for (let i = 0; i < LEVELS.length; i++) {
        if (newTextCount >= LEVELS[i].threshold) {
            newLevel = i + 1
        }
    }
    ...
    if (newLevel > user.level) {
        ...
        const levelReward = levelsGained * 100000
        balanceIncrease += levelReward
        rewards.push({
            type: 'level_up',
            newLevel,
            levelName: LEVELS[newLevel - 1].name,
            amount: levelReward
        })
    }
