...
if (!supabaseUrl || !supabaseKey) {
    console.error('CRITICAL: Supabase URL or Key is missing from .env!')
}
...
export async function incrementTextCount(memberId, groupId, senderName) {
    try {
        ...
        if (updateErr) {
            console.error('❌ Supabase Text Count Update Error:', updateErr)
        }
...
export async function proposeMarriage(senderId, targetId) {
    ...
    if (!cleanTarget || cleanSender === cleanTarget) {
    ...
    await supabase
        .from('marriages')
        .delete()
        .eq('proposer_id', cleanSender)
        .eq('status', 'pending')

    const { error } = await supabase
        .from('marriages')
        .insert([{ proposer_id: cleanSender, target_id: cleanTarget, status: 'pending' }])

    if (error) {
        console.error('Propose marriage error:', error)
        return { error: `Failed to send marriage proposal.` }
    }
...
export async function acceptMarriage(senderId) {
    ...
    if (fetchErr || !proposals || proposals.length === 0) {
        return { error: `You have no pending marriage proposals!` }
    }

    const proposal = proposals[0]
    const cleanProposer = proposal.proposer_id

    const proposer = await getOrCreateUser(cleanProposer)
    if (proposer?.spouse_id) {
        await supabase.from('marriages').update({ status: 'rejected' }).eq('id', proposal.id)
        return { error: `The person who proposed to you is already married!` }
    }
    ...
    await supabase
        .from('marriages')
        .delete()
        .or(`proposer_id.eq.${cleanSender},target_id.eq.${cleanSender},proposer_id.eq.${cleanProposer},target_id.eq.${cleanProposer}`)
        .eq('status', 'pending')
...
export async function divorce(senderId) {
    ...
    return { success: true, splitAmount, spouseId }
}
...
export async function getVault(senderId) {
    ...
    return { vaultBalance: total, spouseId: user.spouse_id }
}
