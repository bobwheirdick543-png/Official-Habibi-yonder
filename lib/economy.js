...
// Router: figures out whether the given key is a weapon, vehicle, or house and
// dispatches to the right purchase flow. Keeps the `.buy <key>` command uniform
// across all three catalogs.
export async function buyShopItem(memberId, itemKey, pushName) {
    const weapon = WEAPON_ITEMS[itemKey]
    if (weapon) return buyWeapon(memberId, weapon, pushName)

    const vehicleGrade = await findVehicleGrade((v) => v.key === itemKey)
    if (vehicleGrade) {
        const user = await getOrCreateUser(memberId, pushName)
    ...
    const houseGrade = await findHouseGrade((h) => h.key === itemKey)
    if (houseGrade) {
        const user = await getOrCreateUser(memberId, pushName)
        return buyGradedAsset({
            memberId, pushName,
...
export async function sellShopItem(memberId, itemKey) {
    const weapon = WEAPON_ITEMS[itemKey]
    if (weapon) return sellWeapon(memberId, weapon)

    if (itemKey === 'car' || (await findVehicleGrade((v) => v.key === itemKey))) {
        const user = await getOrCreateUser(memberId)
    ...
    if (itemKey === 'house' || (await findHouseGrade((h) => h.key === itemKey))) {
        const user = await getOrCreateUser(memberId)
        return sellGradedAsset({
            memberId,
            currentGradeNum: user.house_grade,
...
// .kill has no random component — the 3.5M cost IS the success chance, so it
// always lands unless the target is bulletproof. It wipes the target's
// balance/vehicle/house, forfeits (not splits) their marriage vault, clears
// their inventory, and locks them out of every command for an hour via
...
const KILL_COST = 3500000
const KILL_LOCKOUT_MS = 60 * 60 * 1000
...
// entirely, since .kill is a robbery, not a divorce.
async function forfeitMarriage(memberId) {
    const marriage = await getActiveMarriage(memberId)
    if (!marriage) return
    await supabase
        .from('marriages')
...
export async function killUser(actorId, targetId, groupId, pushName) {
    ...
    await forfeitMarriage(targetId)
