...
// Sell back at a 25% loss — you get 75% of the original price, flat, regardless
// of how many you've bought (no depreciation tracking, keeps it simple).
const SELL_RATE = 0.75

export async function sellShopItem(memberId, itemKey) {
    const item = SHOP_ITEMS[itemKey]
    if (!item) return { error: `No such item. Try \`.inventory\` to see what you own.` }
    ...
    return { item, sellPrice, lostAmount: item.price - sellPrice }
}
