...
// ---------- CREATORS (owner-only admin system) ----------
export async function saveCreator(number) {
    const { error } = await supabase.from('creators').upsert({ whatsapp_number: number })
    if (error) console.error('Error saving creator:', error)
}

export async function isCreator(number) {
    const { data, error } = await supabase.from('creators').select('*').eq('whatsapp_number', number)
