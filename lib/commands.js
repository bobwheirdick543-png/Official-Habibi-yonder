...
export async function handleCommand(sock, msg, groupId, senderId, senderName, command, args, mentionedId) {
    ...
    switch (command) {
        ...
        case 'top': {
            ...
            // Fetch live group metadata to resolve contact push names if missing in DB
            ...
            try {
                ...
                if (meta && meta.participants) {
                    for (const p of meta.participants) {
                        ...
                        if (name) participantMap.set(cleanId, name)
                    }
                }
            } catch (err) {
            ...
            const lines = top.map((u, i) => {
                const rawId = (u.member_id || '').split('@')[0].split(':')[0]
                
                // Prioritize DB push_name (if real name), fallback to WhatsApp group notify name
                ...
                // Clean fallbacks if no push name is available
                if (!displayName || /^\d+$/.test(displayName)) {
                    displayName = `User_${rawId.slice(-4)}`
                }

                return `${i + 1}. ${displayName} — ${formatHabz(u.balance)}`
            })
        ...
        case 'profile': {
            ...
            const calculatedLevel = Math.floor((profile.text_count || 0) / 50) || 1
            return reply(
                `🌵 *${senderName}'s Overrated Stats*\n\n` +
                    `Balance: ${formatHabz(profile.balance)}\n` +
                    `Rank: #${profile.rank}\n` +
                    `Level: Lvl ${calculatedLevel}\n` +
