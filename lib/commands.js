    ...
    switch (command) {
        ...
        case 'top': {
            ...
            const lines = top.map((u, i) => {
                const cleanJid = getCleanJid(u.member_id)
                topMentions.push(cleanJid)
                ...
                const nameLabel = hasCustomName ? ` (${u.push_name})` : ''

                return `${i + 1}. ${getTag(u.member_id)}${nameLabel} : ${formatHabz(u.balance)}`
            })
