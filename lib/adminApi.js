...
// particular state beyond what's checked below. Uses the same forfeitMarriage
// helper as the in-chat .kill so the vault/marriage wipe never drifts out of
// sync between the two paths again.
adminRouter.post('/user/:memberId/kill', async (req, res) => {
    const { memberId } = req.params
    const hours = Number(req.body?.hours) || 1

    await forfeitMarriage(memberId)
    ...
    const { error } = await supabase
        ...
        .update({
            ...
            car_grade: 0,
            house_grade: 0,
            killed_until: new Date(Date.now() + hours * 60 * 60 * 1000).toISOString(),
...
adminRouter.post('/moderators', async (req, res) => {
    ...
    if (!memberId) return res.status(400).json({ error: 'memberId is required' })
    const result = await addModerator(memberId, addedBy || 'admin-panel')
    if (result.error) return res.status(500).json(result)
    res.json({ success: true })
})

adminRouter.delete('/moderators/:memberId', async (req, res) => {
    const result = await removeModerator(req.params.memberId)
    if (result.error) return res.status(500).json(result)
    res.json({ success: true })
})
