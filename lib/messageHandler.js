...
export async function handleIncomingMessage(sock, msg) {
        ...
        for (const reward of rewardInfo.rewards) {
            if (reward.type === 'text_reward') {
                await sock.sendMessage(groupId, {
                    text: `🌵 Look who actually talked enough. ${senderName} hit 300 texts. +${reward.amount.toLocaleString()} Habz.`
                })
                broadcastUpdate('text_reward', { groupId, memberId: senderId, senderName, amount: reward.amount })
            }
            if (reward.type === 'level_up') {
                await sock.sendMessage(groupId, {
                    text: `🌵 ${senderName} leveled up to *${reward.levelName}*. Don't let it go to your head. +${reward.amount.toLocaleString()} Habz.`
                })
    ...
    if (shouldReplyAsAI) {
        console.log(`[AI Triggered] User: ${senderName} | Text: "${text}" | Reason: Reply=${isReply}, Mention=${isMention}, Trigger=${hasTrigger}`)
...
export async function handleGroupParticipantsUpdate(sock, update) {
    ...
    for (const participant of participants) {
        ...
        await sock.sendMessage(groupId, {
            text: `🌵 Great, another baseline human. Welcome @${jid.split('@')[0]}. Try not to embarrass yourself.`,
            mentions: [jid]
        })
    }
}
