        ...
        for (const reward of rewardInfo.rewards) {
            if (reward.type === 'text_reward') {
                await sock.sendMessage(groupId, {
                    text: `Look who actually talked enough. ${getTag(senderId)} hit ${reward.textCount || 300} texts! +${reward.amount.toLocaleString()} Habz.`,
                    mentions: [getCleanJid(senderId)]
                })
            ...
            if (reward.type === 'level_up') {
                ...
                await sock.sendMessage(groupId, {
                    text: `${getTag(senderId)} leveled up to *${levelDisplay}*! Don't let it go to your head. +${reward.amount.toLocaleString()} Habz.`,
                    mentions: [getCleanJid(senderId)]
                })
...
export async function handleGroupParticipantsUpdate(sock, update) {
    ...
    for (const participant of participants) {
        ...
        await sock.sendMessage(groupId, {
            text: `Great, another human joined. Welcome ${tag}. Try not to embarrass yourself.`,
            mentions: [cleanJid]
        })
    }
        }
