            ...
            switch (command) {
                ...
                case 'help': {
                    await reply(
                        ...
                        `• *.strip @user* : Try to strip their steal immunity (10% odds)\n` +
                        `• *.spy @user* : Buy intel on someone's stats (5,000 habz)\n` +
                        `• *.rob @user* : Start a heist crew — needs a car, 50% odds, immunity won't save them\n` +
                ...
                case 'strip': {
                    ...
                    if (result.removedImmunity) {
                        await reply(`✂️ ${senderTag} stripped ${targetDisplayName}'s immunity clean off. Vulnerable now.`, [sender, mentionedJid])
                    } else if (result.success && !result.hadImmunity) {
                        await reply(`✂️ ${senderTag} tried to strip ${targetDisplayName}'s immunity — they didn't even have any. Wasted effort.`, [sender, mentionedJid])
                    } else {
                        await reply(`❌ ${senderTag} tried to strip ${targetDisplayName} and whiffed completely.`, [sender, mentionedJid])
                    }
                    break
                }
                ...
                case 'rob': {
                    ...
                    const ownsCar = await hasItem(sender, 'car')
                    if (!ownsCar) {
                        await reply(`❌ You need a car to run a heist, ${senderTag}. \`.shop\` to buy one.`)
                        break
                    }

                    let ammoBonus = 0
                    if (await hasItem(sender, 'ammo')) {
                        await consumeItem(sender, 'ammo')
                        ammoBonus = 0.15
                    }
                    ...
                    await reply(
                        `🔫 *HEIST TIME* 🔫\n\n${senderTag} is putting together a crew to rob ${targetDisplayName}${ammoBonus > 0 ? ' (loaded up on ammo 🔫)' : ''}. Immunity won't save them.\n\nType \`.join\` in the next 10 seconds to get in (max ${ROB_MAX_CREW}).`,
                        [sender, mentionedJid]
                    )
                    break
                }
                ...
                case 'shop': {
                    ...
                    const lines = items.map((i) => `${i.emoji} *${i.name}* — ${formatHabz(i.price)}\n     ${i.description}`)
                    await reply(`🛒 *Habibi Shop*\n\n${lines.join('\n\n')}\n\nBuy with \`.buy <item>\` (e.g. \`.buy car\`)`)
                    break
                }
                ...
                case 'inv': {
                    ...
                    const lines = items.map((i) => `${i.emoji} *${i.name}*${i.consumable ? ` x${i.quantity}` : ''}`)
